...
public class schooldance {
    ...
    public static void main(String[] args) {
        ...
        schooldance ok = new schooldance();
    ...
    public void solve(InputReader in, PrintWriter w) {
        ...
        int src = students + 2;
        int dst = src + 1;
        MaxFlowDinic flow = new MaxFlowDinic(dst + 10);
        for (int i = 0; i < k; i++) {
            int b = in.ii();
            int g = in.ii() + n;
            flow.addEdge(b, g, 1);
        ...
        for (int i = 1; i <= m; i++)
            flow.addEdge(n + i, dst, 1);
        int f = (int) flow.maxFlow(src, dst);
        w.println(f);
        for (int i = 0; i < f; i++) {
            LinkedList<Integer> p = new LinkedList();
            flow.get_path(src, dst, p);
            p.removeFirst();
            p.removeLast();
            w.println(p.removeFirst() + " " + (p.removeFirst()-n));
        }
    }
