...
public class AdditionOnSegments {
    ...
    public class SegmentTree {
        ...
        public SegmentTree(int n) {
            ...
            Arrays.fill(vals, NO_OPERATION);
            for (int i = vals.length - 1; i >= vals.length - this.size; i--)
                vals[i] = 0;
         ...
         * @param r_idx The right index of user array covered by node x exclusive
         */
        long query(int left, int right, int x, int l_idx, int r_idx) {
            ...
            int m_idx = (l_idx + r_idx) / 2;
            long left_calc = query(left, right, 2 * x + 1, l_idx, m_idx);
            long right_calc = query(left, right, 2 * x + 2, m_idx, r_idx);
    ...
    public static void main(String[] args) {
        ...
        AdditionOnSegments ok = new AdditionOnSegments();
    ...
    public void solve(InputReader in, PrintWriter w) {
        ...
        for (int i = 0; i < m; i++) {
            int t = in.ii();
            if (t == 1) {
                int[] q = in.nextIntArray(3);
                st.modify(q[0], q[1], q[2]);
            } else {
                int[] q = in.nextIntArray(1);
                w.println(st.get(q[0]));
            }
        }
    }
