...
public class b {
    ...
    public void solve(InputReader in, PrintWriter w) {
        ...
        HashMap<Integer, ArrayList<Pair<Double, Double>>> hm = new HashMap();
        HashSet<Circle> consumed = new HashSet();
        for (int i = 0; i < n; i++) {
            ...
            for (Circle prev_circle : circles) {
                if(c.consumes(prev_circle)){
                    
                }
                if (!c.intersect(prev_circle))
                    continue;
