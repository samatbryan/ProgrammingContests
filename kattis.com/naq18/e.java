...
public class e {
    ...
    public void solve(InputReader in, PrintWriter w) {
        ...
        HashMap<Pair<Double, Double>, Integer> centers = new HashMap();
        for (int i = 0; i < n; i++) {
            Pair<Double, Double> c = new Pair(in.nextDouble(), in.nextDouble());
            centers.put(c, centers.getOrDefault(c, 0) + 1);
        }
        for (Pair<Double, Double> c : centers.keySet()) {
            circles.add(new Circle(new Point(c.first, c.second), 1.0, centers.get(c)));
        }
        int res = 0;
        for (int i = 0; i < circles.size(); i++) {
            res = Math.max(res, circles.get(i).count);
            for (int j = 0; j < circles.size(); j++) {
                if (i == j)
                    continue;
                ArrayList<Line> tangents = circles.get(i).tangents(circles.get(j), false);
                if (tangents == null)
                    continue;
                for (Line t : tangents) {
                    int tmp = 0;
