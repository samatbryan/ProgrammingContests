...
public class matchingOnBipartiteGraph {
    ...
    /**
     * Max Flow Dinic Algorithm Tested on CSES: Distinct Routes, School Dance,
     * Download Speed Tested on SPOJ: Fast Maximum Flow
     * 
    ...
    public class MaxFlowDinic {
        ...
        long maxFlow(int src, int dst) {
            ...
            while (bfs(src, dst)) { // while there is a still a valid flow from src to dst
                ...
                while (true) {
                    long flow = sendFlow(src, src, Integer.MAX_VALUE, dst, start);
                    if (flow == 0)
                        break;
                    total_flow += flow;
                }
         ...
         */
        long sendFlow(int src, int cur, long flow, int dst, int[] start) {
            if (cur == dst)
                return flow;
            for (; start[cur] < graph[cur].size(); start[cur]++) {
                ...
                if (this.level[e.node] == this.level[cur] + 1 && e.flow < e.capacity) {
                    ...
                    long candidate_flow = sendFlow(src, e.node, new_flow, dst, start);

                    if (candidate_flow > 0) { // this flow direction was valid
                        if (cur != src) {
                            matches.add(new Pair(cur, e.node));
                        }
    ...
    public static void main(String[] args) {
        ...
        matchingOnBipartiteGraph ok = new matchingOnBipartiteGraph();
    ...
    public void solve(InputReader in, PrintWriter w) {
        ...
        int r = in.ii();
        int m = in.ii();
        MaxFlowDinic flow = new MaxFlowDinic(l + r + 2);
        int src = l + r;
        int dst = src + 1;
        ...
        for (int i = 0; i < m; i++) {
            int left = in.ii();
            int right = in.ii();
            flow.addEdge(left, right + l, 1);
        }
        for (int i = 0; i < l; i++) {
            flow.addEdge(src, i, 1);
        }
        for (int i = 0; i < r; i++) {
            flow.addEdge(i + l, dst, 1);
        }
        int f = (int) flow.maxFlow(src, dst);
        w.println(f);
        for (int i = 0; i < flow.matches.size(); i++) {
            Pair<Integer, Integer> match = flow.matches.get(i);
            w.println(match.first + " " + (match.second - l));
        }
    }
